...
if __name__== '__main__':
    ...
    try:
        while (token==0):
            ...
            Ub = int(M2.split(',')[3])
            # 3.2) compute sa
            hstringa='helloA'+str(PKax)+str(PKay)+str(UAx)+str(UAy)
            md5a=md5()
            ...
            stringb=helloB+str(PKbx)+str(PKby)+str(Ub)
            md5b=md5()
            ...
            if verify(hb,sb,(256,(int(PKbx),int(PKby))))==True:
                start = time.time()
                left = mul(c_p,c_q,c_n,g,Ub)
                right = neg(PKb, c_n)
                sumup = add(c_p,c_q,c_n,left,right)
                Ka=mul(c_p,c_q,c_n,sumup,Ra)
                end = time.time()
